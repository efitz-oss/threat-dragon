...
function createWindow() {
  try {
    ...
    mainWindow.webContents.on('did-fail-load', (event, errorCode, errorDescription) => {
      ...
      dialog.showErrorBox('Page Load Failed', 
    ...
    mainWindow.on('unresponsive', () => {
      ...
      dialog.showErrorBox('Application Not Responding', 
    ...
    if (process.env.NODE_ENV === 'development' && process.env.WEBPACK_DEV_SERVER_URL) {
      ...
      try {
        ...
        try {
          ...
          dialog.showErrorBox('Application Load Failed', 
...
app.whenReady().then(() => {
  try {
    ...
    dialog.showErrorBox('Startup Error', `Failed to start the application: ${startupError.message}`);
  }
}).catch(err => {
