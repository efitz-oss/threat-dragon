...
describe('env/Bitbucket.js', () => {
    ...
    it('has the optional property CLIENT_ID', () => {
        const isRequired = bitbucketEnv.properties.find((x) => x.key === 'CLIENT_ID').required;
        expect(isRequired).to.be.false;
    });

    it('has the optional property CLIENT_SECRET', () => {
        const isRequired = bitbucketEnv.properties.find((x) => x.key === 'CLIENT_SECRET').required;
        expect(isRequired).to.be.false;
    });

    it('has the optional property SCOPE', () => {
        const isRequired = bitbucketEnv.properties.find((x) => x.key === 'SCOPE').required;
        expect(isRequired).to.be.false;
    });
    ...
    it('has the optional property WORKSPACE', () => {
        const isRequired = bitbucketEnv.properties.find((x) => x.key === 'WORKSPACE').required;
        expect(isRequired).to.be.false;
    });
