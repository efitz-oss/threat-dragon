 ...
 * @returns {Boolean}
 */
const isConfigured = () => Boolean(getEnvironment().config.GOOGLE_CLIENT_ID);
...
const getOauthRedirectUrl = () => {
    const scope = getEnvironment().config.GOOGLE_SCOPE || 'openid email profile';
    const redirectUri = getEnvironment().config.GOOGLE_REDIRECT_URI;
    return `https://accounts.google.com/o/oauth2/auth?response_type=code&scope=${scope}&client_id=${getEnvironment().config.GOOGLE_CLIENT_ID}&redirect_uri=${encodeURIComponent(redirectUri)}`;
};
...
const completeLoginAsync = async (code) => {
    ...
    const body = {
        client_id: getEnvironment().config.GOOGLE_CLIENT_ID,
        client_secret: getEnvironment().config.GOOGLE_CLIENT_SECRET,
        code,
        grant_type: 'authorization_code',
        redirect_uri: getEnvironment().config.GOOGLE_REDIRECT_URI,
    };
    const options = {
        headers: {
